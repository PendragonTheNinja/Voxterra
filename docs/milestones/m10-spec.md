...
1. **A3 — no faces toward chunks that will never load.** The mesher needs to
   know which absent neighbours are *never coming* (outside the column's
   window) and treat those as solid rather than air. `Streamer::wants` already
   answers "is this neighbour coming?" for the first-mesh gate — reuse it,
   don't re-derive it. The camera window adds new such boundaries: the top and
   bottom of the camera's neighbourhood underground or aloft, and the gap
   between it and the surface window.
2. **A3 — `column_heights` pruning** on chunk-column unload, and removal of the
   redundant level-0 LOD real-height gather.
3. **A3 — sparse LOD sampler** beyond level 0, with an ADR-0008 amendment
...
4. **A3 — LOD grid lines. Diagnose before fixing.** Faint straight lines run
...
5. **A3 — cleanup.** Delete the orphaned `vox-core/src/downsample.rs` and the
...
6. **A1 — transparency and water** (ADR-0011, Accepted, all decisions taken:
   ...
   oceans meshed.
7. **Tuning and retro.** Criterion-8 numbers at radius 8, on foot and flying,
